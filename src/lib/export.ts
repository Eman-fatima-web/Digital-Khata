...
export function toCsv(rows: Record<string, unknown>[]): string {
  ...
  const escape = (v: unknown) => {
    ...
    // CSV formula-injection guard: cells beginning with = + - @ (or a
    // leading tab/carriage return) are interpreted by spreadsheets as
    // formulas. Prefixing with a single quote neutralizes them.
    if (/^[=+\-@\t\r]/.test(s)) {
      s = `'${s}`
    }
