...
import { appOverview, explainFeature, formatFeature, isSecurityQuestion, securityAwareness } from './knowledge'
...
export function runEngine(
  ...
  switch (intent) {
    ...
    case 'GREETING':
      return { type: 'answer', text: r.greeting(data) }
    ...
    case 'HELP':
      return { type: 'answer', text: r.help(data) }

    case 'APP_INFO': {
      const kb = explainFeature(input)
      if (kb) return { type: 'answer', text: formatFeature(kb, language) }
      if (isSecurityQuestion(input)) return { type: 'answer', text: securityAwareness(language) }
      return { type: 'answer', text: appOverview(language) }
    }
...
const PAGE_MAP: Record<string, string> = {
  ...
  products: '/products', product: '/products', 'مصنوعات': '/products',
  ai: '/ai', assistant: '/ai',
}
