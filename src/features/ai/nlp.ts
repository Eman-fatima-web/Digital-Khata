...
const GREETING_PATTERNS = [
  ...
  /آداب/,
  /ہیلو/,
  /\bassalam\s*(?:o|u)?\s*alaikum\b/i,
  /\b(?:salam|salaam|adaab)\b/i,
  /\b(?:kia|kya|kaise|kese|kaisi|kesi)\s+(?:ha?al|kaam|halaat)\b/i,
  /\b(?:kaise|kese|kaisi|kesi)\s+ho\b/i,
]
...
export function detectMessageLanguage(input: string): 'en' | 'ur' | 'ur-script' {
  ...
  // Urdu-script markers — word-boundary safe within a fixed word list
  const urduScriptIndicators = /\b(آج|آہ|کی|کو|ہے|ہیں|میں|نہیں|کیا|کے|اور|پر|سے|ہاں|بھائی|بناؤ|شامل|دیا|دیتا|رہا|رہی|ہوں|گی|گا|کرو|کر|جائیں|آؤ|جاؤ|شکریہ|مہربانی|بارہ)\b/i
  if (urduScriptIndicators.test(normalized)) return 'ur'

  // Roman Urdu markers — high-signal English-alphabet words. Collisions with
  // genuine English ("car" vs "kar", "he" vs "hе") are avoided by not listing
  // such words; single matches therefore indicate Roman Urdu.
  const romanUrduIndicators = /\b(?:kia|kya|hai|hy|hain|hae|ho|hoga|hogi|honge|hoon|mujhe|mujh|mera|meri|mere|tera|tumhara|tumhari|apna|apni|apne|batao|bataen|btao|karo|karein|karna|karke|kro|kren|udhaar|udaar|khata|kitna|kitni|kitne|kaun|kiska|kiski|kiske|kahan|kyun|kion|shukriya|bilkul|madad|theek|thik|salaam|adaab|chahiye|nahi|nhi|diya|dena|liya|lena|raha|rahi|hum|aap|haan|kuch|yeh|woh|ko|ka|ke|ki)\b/i
