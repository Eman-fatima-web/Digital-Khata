...
export function getCloudAIConfig(): CloudAIConfig {
  ...
  // Cloud AI is enabled by default. When online, unknown queries reach the
  // smart server-side LLM (OpenRouter) through the same-origin '/api' proxy
  // (no VITE_API_BASE_URL needed — this keeps LAN/phone access working).
  // Set VITE_AI_ENABLED=false to force local-only AI.
  return {
    enabled: enabledValue === undefined ? true : enabledValue === 'true' || enabledValue === '1',
