...
type Responses = {
  ...
  greeting: (data?: KhataSnapshot) => string
  help: (data?: KhataSnapshot) => string
  appInfo: () => string
...
const en: Responses = {
  ...
  greeting: (data) => {
    const names = (data?.customers ?? []).map((c) => c.name).filter(Boolean)
    const outstanding = (data?.udhaar ?? []).reduce((s, e) => s + e.remainingAmount, 0)
    const liveStat =
      names.length > 0
        ? `I can already see your khata — you have ${names.length} customer${names.length > 1 ? 's' : ''} and ${formatCurrency(outstanding)} outstanding in udhaar. `
        : ''
    return (
      liveStat +
      `Assalam-o-Alaikum! I am Khata AI, your shop assistant. I do much more than record payments — I can read your balances, top debtors, overdue, today's/weekly/monthly sales, reports, reminders, and I can also answer general questions: pricing tips, market trends, business advice, translations — in English or Urdu. Just ask me anything, e.g. "Ahmed balance", "5000 udhaar do", or "kitna profit per saman rakhun?"`
    )
  },
  help: (data) => {
    const names = (data?.customers ?? []).map((c) => c.name).filter(Boolean)
    if (names.length === 0) {
      return 'Your khata is empty — start building it:\n• "Add new customer"\n• "Give 5000 udhaar to <name>"\n• "Receive 1000 payment from <name>"\n• "Record sale 3000"\n\nOr ask me anything — business tips, pricing, market advice.'
    }
    const [a, b] = names
    const list = names.slice(0, 3).join(', ')
    return `You have ${names.length} customer${names.length > 1 ? 's' : ''} — ${list}. Ask me about YOUR khata:\n• "${a} balance"\n• "Top debtor"\n• "Today's sales"\n• "Business overview"\n\nOr record something:\n• "${b ?? a} ko 5000 udhaar do"\n• "${a} se 1000 payment lo"\n• "Record sale 3000"\n• "Add new customer"`
  },
  appInfo: () =>
    'Digital Khata is a ledger (udhaar) app for shopkeepers. It tracks customers, credit given, payments received, and sales — all offline.\n\nYou can: manage customers, record udhaar and payments, log daily sales, view daily/weekly/monthly reports, print 80mm receipts or PDF reports, send WhatsApp/SMS payment reminders, set a PIN lock, and switch between English and Urdu.\n\nJust ask me in English or Urdu, e.g. "Ahmed balance" or "اس مہینے کی فروخت کتنی ہے؟".',
...
const ur: Responses = {
  ...
  greeting: (data) => {
    ...
    const liveStat =
      names.length > 0
        ? `میں آپ کا کھاتہ سمجھ چکا ہوں — آپ کے ${names.length} گاہک ہیں اور ${formatCurrency(outstanding)} ادھار بقایا ہے۔ `
        : ''
    return (
      liveStat +
      'وعلیکم السلام! میں خاتہ AI ہوں، آپ کے دکان کا معاون۔ میں صرف ادائیگی ریکارڈ نہیں کرتا — بیلنس، سب سے بڑا مقروض، بقایا، آج/ہفتہ/مہینہ کی فروخت، رپورٹس، یاد دہانیاں، اور ساتھ ہی عام سوالات بھی: قیمتوں کے مشورے، مارکیٹ کے رجحانات، کاروباری رہنمائی، ترجمہ — انگریزی یا اردو میں۔ بس کچھ بھی پوچھیں، جیسے "احمد کا بیلنس"، "5000 ادھار دو"، یا "سامان پر کتنا منافع رکھوں؟"'
    )
  },
  help: (data) => {
    const names = (data?.customers ?? []).map((c) => c.name).filter(Boolean)
    if (names.length === 0) {
      return 'آپ کا کھاتہ ابھی خالی ہے — بنانا شروع کریں:\n• "نیا گاہک شامل کرو"\n• "<نام> کو 5000 ادھار دو"\n• "<نام> سے 1000 وصول کرو"\n• "آج کی فروخت لکھو"\n\nیا مجھ سے کچھ بھی پوچھیں — کاروبار، قیمتوں، یا مشورے کے بارے میں۔'
    }
    const [a, b] = names
    const list = names.slice(0, 3).join('، ')
    return `آپ کے کھاتے میں ${names.length} گاہک ہیں — ${list}۔ اصل ناموں کے ساتھ پوچھیں:\n• "${a} کا بیلنس"\n• "سب سے زیادہ ادھار کس کا ہے؟"\n• "آج کی فروخت کتنی ہے؟"\n• "کاروبار کیسا چل رہا ہے؟"\n\nیا کچھ لکھوائیں:\n• "${b ?? a} کو 5000 ادھار دو"\n• "${a} سے 1000 وصول کرو"\n• "آج کی فروخت لکھو"\n• "نیا گاہک شامل کرو"`
  },
  appInfo: () =>
    'ڈیجیٹل خاتہ دکانداروں کے لیے ایک کھاتہ (ادھار) ایپ ہے۔ یہ آف لائن کام کرتی ہے اور گاہکوں، ادھار، وصولی اور فروخت کا ریکارڈ رکھتی ہے۔\n\nآپ یہ سب کر سکتے ہیں: گاہک کا انتظام، ادھار اور ادائیگی ریکارڈ کرنا، روزانہ فروخت لکھنا، روزانہ/ہفتہ/مہینہ کی رپورٹ، پرنٹ رسیٹ یا PDF رپورٹ، واٹس ایپ/ایس ایم ایس سے یاد دہانی، PIN لاک، اور اردو/انگریزی زبان۔\n\nبس مجھ سے انگریزی یا اردو میں پوچھیں، مثلاً "احمد کا بیلنس" یا "اس مہینے کی فروخت کتنی ہے؟"۔',
