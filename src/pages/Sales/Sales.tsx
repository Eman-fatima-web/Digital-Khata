...
import { saveProducts, decrementProductStock } from '../../data/services/savedProducts'
import { formatCurrency, formatDate, localDateKey } from '../../lib/utils'
import { billTotal, hasValidItems } from '../../lib/bill'
import { BillItemsEditor } from '../../components/BillItemsEditor'
import { Button } from '../../components/ui/Button'
import { Sheet } from '../../components/ui/Sheet'
import { EmptyState } from '../../components/ui/EmptyState'
import { PageLoader } from '../../components/ui/PageLoader'
import type { Customer, BillItem } from '../../core/types'
...
function Sales() {
  ...
  const handleAddSale = async () => {
    ...
    if (!description.trim() || (!hasItems && (!numericAmount || numericAmount <= 0))) return

    const finalItems = hasItems ? items.filter((i) => i.name.trim() !== '' && i.qty > 0 && i.rate > 0) : undefined
      ...
      await addSale(
        {
          ...
          amount: hasItems ? billTotal(finalItems!) : numericAmount,
          date,
          items: finalItems,
        },
        owner,
      )
      if (hasItems && finalItems) {
        saveProducts(
          owner.shopId,
          finalItems.filter((i) => i.name.trim() !== '').map((i) => ({ name: i.name, rate: i.rate })),
        )
        decrementProductStock(
          owner.shopId,
          finalItems.filter((i) => i.name.trim() !== '').map((i) => ({ name: i.name, qty: i.qty })),
        )
      }
      setCustomerId('')
      setDescription('')
      ...
      >
        <div className="space-y-5">
          ...
          <BillItemsEditor
            ...
            items={items}
            onChange={setItems}
            onTotalExpected={(total) => setAmount(String(total))}
          />
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                type="number"
                min="1"
                value={hasValidItems(items) ? billTotal(items) : amount}
                onChange={(e) => setAmount(e.target.value)}
                disabled={hasValidItems(items)}
                placeholder="0"
                className="h-12 w-full rounded-xl border border-surface-hairline bg-surface-card pl-12 pr-4 text-sm outline-none transition focus:border-success-300 focus:ring-4 focus:ring-success-400 disabled:opacity-60"
              />
            </div>
            {hasValidItems(items) && (
              <p className="mt-1 text-xs text-ink-muted">Auto-calculated from items.</p>
            )}
          </div>
          ...
          <div className="flex flex-col-reverse gap-3 pt-1 sm:flex-row sm:justify-end">
            ...
            <Button
              ...
              disabled={!description.trim() || (!amount && !hasValidItems(items)) || isSubmitting}
