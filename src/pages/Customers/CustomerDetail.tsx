...
export default function CustomerDetail() {
  ...
  const handleDownloadStatement = () => {
    ...
    ;(udhaar ?? []).forEach((entry) => {
      const itemList = entry.items && entry.items.length > 0
        ? ` [${entry.items.filter((i) => i.name.trim() !== '').map((i) => `${i.name} (${i.qty}×${i.rate})`).join(', ')}]`
        : ''
      rows.push({
        date: entry.createdAt,
        description: `Udhaar — ${entry.description || 'Credit'}${itemList}`,
        debit: entry.amount,
        credit: 0,
      })
    })
    ...
    ;(sales ?? []).forEach((sale) => {
      const itemList = sale.items && sale.items.length > 0
        ? ` [${sale.items.filter((i) => i.name.trim() !== '').map((i) => `${i.name} (${i.qty}×${i.rate})`).join(', ')}]`
        : ''
      rows.push({
        date: sale.date || sale.createdAt,
        description: `Sale — ${sale.description || 'Sale'}${itemList}`,
        debit: 0,
        credit: sale.amount,
      })
    })

    rows.sort((a, b) => new Date(a.date).valueOf() - new Date(b.date).valueOf())
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {entry.items && entry.items.length > 0 && (
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        {entry.items.filter((i) => i.name.trim() !== '').map((i) => (
                          <span key={i.name} className="inline-flex items-center gap-1 rounded-full bg-surface px-2 py-0.5 text-[11px] text-ink-muted">
                            {i.name}
                            <span className="font-semibold text-ink-light">{i.qty}×{formatCurrency(i.rate)}</span>
                          </span>
                        ))}
                      </div>
                    )}
                ...
                <div key={sale.id} className="flex items-center gap-4 px-5 py-4 sm:px-6">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {sale.items && sale.items.length > 0 && (
                      <div className="mt-1.5 flex flex-wrap gap-1.5">
                        {sale.items.filter((i) => i.name.trim() !== '').map((i) => (
                          <span key={i.name} className="inline-flex items-center gap-1 rounded-full bg-surface px-2 py-0.5 text-[11px] text-ink-muted">
                            {i.name}
                            <span className="font-semibold text-ink-light">{i.qty}×{formatCurrency(i.rate)}</span>
                          </span>
