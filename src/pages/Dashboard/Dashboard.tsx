...
import { getSavedProducts } from '../../data/services/savedProducts'
...
function Dashboard() {
  ...
  const profitData = useMemo(() => {
    const products = getSavedProducts(owner.shopId)
    ...
    return { revenue, cost: costTotal, profit: revenue - costTotal }
  }, [sales, owner.shopId])
  ...
  const weeklySalesTrend = useMemo(() => {
    ...
    for (const s of sales ?? []) {
      ...
      if (d < start || d >= new Date(today.getTime() + 24 * 60 * 60 * 1000)) continue
      const key = localDateKey(d)
      map.set(key, (map.get(key) ?? 0) + s.amount)
    }
    for (let d = new Date(start); d < new Date(today.getTime() + 24 * 60 * 60 * 1000); d.setDate(d.getDate() + 1)) {
      days.push({ label: d.toLocaleDateString('en-PK', { weekday: 'short' }), value: map.get(localDateKey(d)) ?? 0 })
    }
    return days
  }, [sales])
  ...
  const topDebtors = useMemo(() => {
    ...
    for (const e of udhaar ?? []) {
      if (e.remainingAmount <= 0) continue
      balances.set(e.customerId, (balances.get(e.customerId) ?? 0) + e.remainingAmount)
    }
  ...
  const udhaarVsPayments = useMemo(() => {
    const now = new Date()
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1)
    const monthEnd = new Date(now.getFullYear(), now.getMonth() + 1, 1)
    const udhaarMonth = (udhaar ?? []).filter((e) => {
      const d = new Date(e.createdAt)
      return d >= monthStart && d < monthEnd
    })
    const payMonth = (payments ?? []).filter((p) => {
      const d = new Date(p.date)
      return d >= monthStart && d < monthEnd
    })
    return {
      udhaar: udhaarMonth.reduce((sum, e) => sum + e.amount, 0),
      payments: payMonth.reduce((sum, p) => sum + p.amount, 0),
    }
  }, [udhaar, payments])
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <section className="grid grid-cols-2 gap-3 sm:grid-cols-2 xl:grid-cols-4 sm:gap-4">
        ...
        <StatCard
          ...
          icon={TrendingUp}
          iconClassName={profitData.profit >= 0 ? 'bg-success-50 text-success-500' : 'bg-danger/10 text-danger'}
