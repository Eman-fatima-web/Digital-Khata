 ...
 * Exchange a valid (even near-expired) auth token for a fresh one.
 * Enables the sync layer to recover from mid-sync token expiry without
...
authRouter.post('/refresh', async (req, res) => {
  try {
    ...
    const freshToken = generateToken(decoded.userId, decoded.businessId, decoded.role)
  ...
  try {
    ...
    if (useDb) {
      const result = await query(
        `SELECT u.*, b.id as business_id, b.name as business_name FROM users u 
      ...
      const token = generateToken(user.id, user.business_id, user.role || 'user')
      ...
      res.json({
        ...
        user: {
          ...
          shopName: user.shop_name || user.business_name,
...
const passwordHash = await bcrypt.hash(password, 10)
        ...
        const isAdminEmail = !isSuperAdminEmail && (process.env.ADMIN_EMAIL || '')
          .split(',')
          .map((e) => e.trim().toLowerCase())
          .filter(Boolean)
          .includes(normalizedEmail)
        const userRole = isSuperAdminEmail ? 'superadmin' : isAdminEmail ? 'admin' : 'user'
        ...
        const token = generateToken(userId, businessId, userRole)
