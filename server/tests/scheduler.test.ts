...
describe('Scheduler', () => {
  it('starts and reports active jobs', async () => {
    ...
    expect(jobs[1].name).toBe('weeklySummary')
    expect(jobs[1].schedule).toBe('0 9 * * 1')
    expect(jobs[1].active).toBe(true)
    expect(jobs[2].name).toBe('monthlySummary')
    expect(jobs[2].schedule).toBe('0 9 1 * *')
    expect(jobs[2].active).toBe(true)
    ...
    expect(afterStop.every((j) => !j.active)).toBe(true)
  })
})
