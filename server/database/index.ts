...
export async function isDatabaseAvailable(): Promise<boolean> {
  ...
  // Re-verify connectivity periodically (every 30s) instead of caching a
  // transient failure forever — a cold-start pool connection can time out
  // once and then work fine on the next attempt.
  const now = Date.now()
  if (dbAvailable === true && now - lastCheck < 30_000) return true
  try {
