...
export async function createUser(
  ...
  saveStore(store)
  log.info({ email, userId: user.id, role: user.role }, 'Local auth: user registered')
  return user
}
...
export function setUserRole(userId: string, role: 'user' | 'admin' | 'superadmin'): LocalUser | undefined {
  const store = loadStore()
  const user = store.users.find((u) => u.id === userId)
  if (!user) return undefined
  user.role = role
...
export function setUserActive(userId: string, isActive: boolean): LocalUser | undefined {
  const store = loadStore()
  const user = store.users.find((u) => u.id === userId)
  if (!user) return undefined
...
export function setUserVerified(userId: string, emailVerified: boolean): LocalUser | undefined {
  const store = loadStore()
  const user = store.users.find((u) => u.id === userId)
  if (!user) return undefined
...
export function isUserAdmin(user: LocalUser | undefined): boolean {
  return user?.role === 'admin'
}

export function toPublicUser(user: LocalUser) {
  return {
    id: user.id,
    email: user.email,
