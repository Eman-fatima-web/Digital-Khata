...
async function getDailyData(businessId: string, dateStr: string) {
  const [salesResult, paymentsResult, udhaarResult, outstandingResult, overdueResult] = await Promise.all([
    query(
      `SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
       FROM sales WHERE business_id = $1 AND is_deleted = FALSE AND date = $2`,
      [businessId, dateStr],
    ),
    query(
      `SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
       FROM payments WHERE business_id = $1 AND is_deleted = FALSE AND date = $2`,
      [businessId, dateStr],
    ),
    query(
      `SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
       FROM udhaar WHERE business_id = $1 AND is_deleted = FALSE AND DATE(created_at) = $2`,
      [businessId, dateStr],
    ),
    query(
...
export async function runDailySummaryJob(): Promise<{ sent: number; failed: number; skipped: number }> {
  const results = { sent: 0, failed: 0, skipped: 0 }

  try {
    const businesses = await getBusinessesWithVerifiedEmails()
    log.info({ count: businesses.length }, 'Running daily summary for businesses')

    const today = new Date().toISOString().split('T')[0]

    for (const business of businesses) {
      try {
        const data = await getDailyData(business.id, today)
        ...
        if (sent) {
          ...
          await query(
            `INSERT INTO audit_logs (business_id, action, tool_name, status, details)
             VALUES ($1, 'daily_summary_sent', 'scheduler', 'success', $2)`,
            [business.id, JSON.stringify({ email: business.ownerEmail, date: today })],
          )
        } else {
          results.skipped++
        }
        ...
        results.failed++
        log.error({ err, businessId: business.id }, 'Failed to send daily summary')
      }
    }
  } catch (err) {
  ...
  log.info(results, 'Daily summary job completed')
  return results
}
