...
import { buildWeeklySummaryEmail, type DailySummaryData } from './emailTemplates.js'
import { createChildLogger } from '../logger.js'

const log = createChildLogger({ module: 'weekly-summary-job' })
...
async function getWeeklyData(businessId: string) {
  const [salesResult, paymentsResult, udhaarResult, outstandingResult, overdueResult] = await Promise.all([
    query(
      `SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
       FROM sales WHERE business_id = $1 AND is_deleted = FALSE AND date >= CURRENT_DATE - INTERVAL '7 days'`,
      [businessId],
    ),
    query(
      `SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
       FROM payments WHERE business_id = $1 AND is_deleted = FALSE AND date >= CURRENT_DATE - INTERVAL '7 days'`,
      [businessId],
    ),
    query(
      `SELECT COALESCE(SUM(amount), 0) as total, COUNT(*) as count
       FROM udhaar WHERE business_id = $1 AND is_deleted = FALSE AND created_at >= CURRENT_DATE - INTERVAL '7 days'`,
      [businessId],
    ),
    query(
...
export async function runWeeklySummaryJob(): Promise<{ sent: number; failed: number; skipped: number }> {
  const results = { sent: 0, failed: 0, skipped: 0 }

  try {
    const businesses = await getBusinessesWithVerifiedEmails()
    log.info({ count: businesses.length }, 'Running weekly summary for businesses')
    ...
    for (const business of businesses) {
      try {
        const data = await getWeeklyData(business.id)
        ...
        if (sent) {
          ...
          await query(
            `INSERT INTO audit_logs (business_id, action, tool_name, status, details)
             VALUES ($1, 'weekly_summary_sent', 'scheduler', 'success', $2)`,
        ...
        results.failed++
        log.error({ err, businessId: business.id }, 'Failed to send weekly summary')
      }
    }
  } catch (err) {
  ...
  log.info(results, 'Weekly summary job completed')
  return results
}
