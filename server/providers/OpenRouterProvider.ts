...
/**
 * OpenRouter AI Provider with automatic multi-key fallback.
 *
 * Env vars (in priority order):
 *   OPENROUTER_API_KEY     — primary key
 *   OPENROUTER_API_KEY_2   — first fallback (used on 401/429/5xx from primary)
 *   OPENROUTER_API_KEY_3   — second fallback
 *
 * If all configured keys fail, the error is thrown so callers can fall back
 * further down the chain (e.g. to the Mock/local provider) instead of showing
...
export class OpenRouterProvider implements AIProvider {
  ...
  async answer(request: AIProviderRequest): Promise<AIProviderResponse> {
    ...
    // Iterate over keys; on an auth/rate/server error, try the next key.
    for (let i = 0; i < this.keys.length; i++) {
      const apiKey = this.keys[i]
      try {
        ...
        if (this.retryable(err)) {
          log.warn({ keyIndex: i, message: err.message }, `OpenRouter key ${i + 1} failed; trying next key`)
          continue
        }
        throw err
      }
  ...
  private retryable(err: Error): boolean {
    return (
